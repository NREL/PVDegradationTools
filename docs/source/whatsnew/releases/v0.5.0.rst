...
- Documentation overhaul. Significant ``User Guide`` improvements. Added geospatial information with visual aids, added meteorological data page and materials access page.
...
- ``decorators.py`` namespace changed to default ``pvdeg`` namespace. now this can be directly accessed via ``pvdeg.decorators``. This reduces the need for an extra import.
    -  ``geospatial_quick_shape`` decorator namespace changed to default pvdeg namespace
...
Bug Fixes
-------------
- fixed an issue where pvdeg.geospatial.elevation_stochastic_downselect would return metadata row indices which were downselected instead of named gids.
