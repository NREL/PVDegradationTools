...
def test_temperature():
    ...
    weather_args = {'poa_global':poa['poa_global'], 'temp_air':WEATHER['temp_air'], 'wind_speed':WEATHER['wind_speed']}

    pvlib_df = pd.DataFrame()
    result_df = pd.DataFrame()

    sapm_parameters = pvlib.temperature.TEMPERATURE_MODEL_PARAMETERS['sapm']['open_rack_glass_polymer']    
    ...
    pvlib_df['faiman'] = pvlib.temperature.faiman(
        **weather_args
    )
    ...
    pvlib_df['faiman_rad'] = pvlib.temperature.faiman_rad(
        **weather_args
    )
    ...
    pvlib_df['fuentes'] = pvlib.temperature.fuentes(
        **weather_args,
        noct_installed=45
    )    
    ...
    pd.testing.assert_frame_equal(result_df, pvlib_df, check_dtype=False,check_names=False,check_like=False)
