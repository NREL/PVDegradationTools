...
def _mountains(meta_df, kdtree, index, rad_1, rad_2, threshold_factor):
    coordinates = meta_df[['latitude', 'longitude']].values
    elevations = meta_df['altitude'].values
...
# TODO: add altitude floor cutoff arg
def identify_mountains(meta_df, kdtree, rad_1=12, rad_2=1, threshold_factor=1.25):
    """
    ...
    --------
    meta_df : pd.DataFrame
        Modified dataframe with a new column ``is_mountain``. Boolean for 
        each entry in the dataframe characterizing mountainous vs flat.
    """

    meta_df.loc[:,'is_mountain'] = [_mountains(meta_df, kdtree, i, rad_1, rad_2, threshold_factor) for i in range(len(meta_df))]

    return meta_df

def identify_coastline(meta_df, kdtree, radius=1):
    """
    Find coastlines in a set of metadata using sklearn tree for fask lookup.
    Uses cartopy coastline shapes to extract coastline coordinates.
    ...
    radius : float, default = 1
        kdtree query radius, effectively controls width of downselected data.
        This does not map to a geographical distance yet. Number is arbitrary
    
    Returns:
    --------
    meta_df : pd.DataFrame
       Modified dataframe with a new column ``coastline``. Boolean for 
        each entry in the dataframe characterizing coastline vs inland.
    """
    ...
    meta_df.iloc[include_arr, meta_df.columns.get_loc('coastline')] = True
