...
def identify_mountains(meta_df, kdtree, rad_1=12, rad_2=1, threshold_factor=1.25, elevation_floor=0, bbox_kwarg={}):
    """
...
def feature_downselect(meta_df, kdtree=None, feature_name=None, resolution='10m', radius=None, bbox_kwarg={}):
    """
    meta_df : pd.DataFrame
        Dataframe of metadata as generated by pvdeg.weather.get for geospatial
...
def apply_bounding_box(meta_df, coord_1=None, coord_2=None, coords=None):
    """
...
def elevation_stochastic_downselect(
    ...
    m = int( len(elevations) * 0.5 )
    weights = np.empty_like(elevations)

    for i, coord in enumerate(coords):
        indicies = kdtree.query(coord.reshape(1,-1), k=k_neighbors+1)[1][0] # remove +1
        delta_elevation = np.abs(elevations[indicies[1:]] - elevations[i]) # remove getting rid of current elevation instead just never take it out
    ...
    normalized_weights = np.empty_like(weights)
