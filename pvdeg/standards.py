...
def interpret_standoff( standoff_1=None, standoff_2=None ):
    ...
    if standoff_1 is not None:
        ...
        if standoff_2 is not None:
            ...
            try:
                x80 = -(-x70 /( np.log(1 - (T98_0 - 70) / (T98_0 - T98_inf)))) * np.log(1 - (T98_0 - 80) / (T98_0 - T98_inf))
            except RuntimeWarning as e:
                x80=None         
    else:
        x70=None
    ...
    if x70 == None:
        ...
        if T98_0 is not None:
            Output = "The estimated temperature of an insulated-back module is " + '%.1f' % T98_0 + '°C. \n'
        if T98_inf is not None:
            Output = Output + 'The estimated temperature of an open-rack module is ' + '%.1f' % T98_inf + '°C. \n'
        if x80 == None: 
            Output = Output + 'The minimum standoff for Level 0 certification and T₉₈<70°C is ' + '%.1f' % x70 + ' cm.'
        else:    
            Output = Output + 'Level 0 certification is valid for a standoff greather than ' + '%.1f' % x70 + ' cm. \n'
            if x70 > 0: 
                if x80 > 0:
                    Output = Output + 'Level 1 certification is required for a standoff between than ' + '%.1f' % x70  + ' cm, and ' + '%.1f' % x80 + ' cm. \n'
                    Output = Output + 'Level 2 certification is required for a standoff less than ' + '%.1f' % x80  + ' cm.'
                else:
                    Output = Output + 'Level 1 certification is required for a standoff less than ' + '%.1f' % x70  + ' cm. \n'
...
def standoff_x(weather_df, meta, tilt, azimuth, sky_model, 
    temp_model=None, conf_0=None, conf_inf=None, T98=None, x_0=None, wind_factor=None,):
    ...
    temp_df=standoff(weather_df=weather_df, meta=meta, tilt=tilt, azimuth=azimuth,
    sky_model=sky_model, temp_model=temp_model, conf_0=conf_0, conf_inf=conf_inf, T98=T98, x_0=x_0, 
    wind_factor=wind_factor).x[0] 
