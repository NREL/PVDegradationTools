...
def test_calc_letid_outdoors():
    ...
    generation_df = pd.read_excel(os.path.join(DATA_DIR, 'PVL_GenProfile.xlsx'), header = 0) # this is an optical generation profile generated by PVLighthouse

    result = letid.calc_letid_outdoors(tau_0, tau_deg,wafer_thickness, s_rear, nA_0, nB_0, nC_0, WEATHER, META, mechanism_params, generation_df, module_parameters = cec_module)

    pd.testing.assert_frame_equal(result, OUTDOORS_TIMESTEPS)
...
def test_calc_letid_lab():
    ...
    pd.testing.assert_frame_equal(result, LAB_TIMESTEPS)
