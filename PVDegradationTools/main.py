...
class Degradation:
    ...
    def carrier_factor(
        ...
        dn = Degradation.calc_dn(tau, temperature, suns, jsc, wafer_thickness, s_rear)
    ...
    def calc_dn(
        ...
        i_applied = suns * jsc
        v_applied = Degradation.convert_i_to_v(
    ...
    def convert_i_to_v(tau, na, current, wafer_thickness, srv, temperature=298.15, e_mobility=0.15,
        ...
        j0 = Degradation.j0_gray(ni2, diffusivity, na, diffusion_length, arg, srv)
    ...
    def calc_voc_from_tau(tau, wafer_thickness, srv_rear, jsc, temperature, na=7.2e21):
        ...
        return(Degradation.convert_i_to_v(tau, na, jsc, wafer_thickness, srv_rear, temperature))
    ...
    def calc_device_params(timesteps, cell_area=239):
        ...
        timesteps.loc[:, "FF"] = Degradation.ff_green(timesteps.loc[:, "Voc"])
        timesteps.loc[:, "Pmp"] = (
            timesteps.loc[:, "Voc"]
            * timesteps.loc[:, "FF"]
            * timesteps.loc[:, "Jsc"]
    ...
    def calc_pmp_loss_from_tau_loss(tau_0, tau_deg, cell_area, wafer_thickness, s_rear,
        ...
        if generation is None or depth is None:
            generation_df = pd.read_excel('..\..\letid\data\PVL_GenProfile.xlsx', header = 0,
                                          engine="openpyxl")
        ...
        voc_0 = Degradation.calc_voc_from_tau(
            tau_0, wafer_thickness, s_rear, jsc_0, temperature=25, na=7.2e21
        )
        voc_deg = Degradation.calc_voc_from_tau(
            tau_deg, wafer_thickness, s_rear, jsc_deg, temperature=25, na=7.2e21
        )

        ff_0 = Degradation.ff_green(voc_0)
        ff_deg = Degradation.ff_green(voc_deg)
    ...
    def ff_green(voltage, temperature=298.15):
        ...
        temperature : numeric, defaul 298.15
